...
import SimpleModalTimeline from "../components/SimpleModalTimeline";
import ExportJsonExcel from 'js-export-excel';

const RelatorioLead = (props) => {
  ...
  const updateStatus = async (statusModalValues) => { 
	...
	let dropdownInfo;
    await fetch(
	  process.env.test_api + "lead/log/" + statusModalValues.id,
	  {
		  headers: {
	...
	for (let i in data) {
	  if(data){
	    let obj = {
	                 ...
	                 'E-mail': data[i].email,
	                 'Telefone Fixo': data[i].telefone_fixo,
	                 'Telefone': data[i].telefone_celular,
	                 'Qtd Beneficiarios': data[i].qtde_beneficiarios,
	 ...
	 option.fileName = 'Relatorio - Leads'
	option.datas=[
	{
	  sheetData:dataTable,
	  sheetName:'sheet',
	         sheetFilter:['id', 'Nome', 'CPF/CNPJ', 'Estado', 'Contato', 'E-mail', 'Telefone Fixo', 'Telefone', 'Qtd Beneficiarios', 'Status', 'Detalhes'],
	         sheetHeader:['id', 'Nome', 'CPF/CNPJ', 'Estado', 'Contato', 'E-mail', 'Telefone Fixo', 'Telefone', 'Qtd Beneficiarios', 'Status', 'Detalhes'],
	}
	];
  ...
  return (
    <div>
		<Card title="Relatorio - Leads" extra={<Button onClick={() => downloadExcel()}>Export Excel Table</Button>}>
		  <Row>
			<Col span={24}>          
			  <Table
...
export default connect(mapStateToProps)(RelatorioLead);
